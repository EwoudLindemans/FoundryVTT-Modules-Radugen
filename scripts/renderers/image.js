...
radugen.renderer.Image = class {
    ...
    async render() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (this._theme.background.length) {
                ...
                await this.applyGenericFilters(backgroundCtx, this._theme.settings.background);
            ...
            if (this._theme.floor.length) {
                ...
                await this.applyGenericFilters(floorCtx, this._theme.settings.floor, gradient, gradientDirection);
            ...
            if (this._theme.wall.length) {
                ...
                await this.applyGenericFilters(wallCtx, this._theme.settings.wall, gradient, gradientDirection);
    ...
    async applyGenericFilters(ctx, themeSettings, gradient, gradientDirection) {
        if(typeof themeSettings.gradient === 'object'){
            let s = themeSettings.gradient;
            let rib = radugen.helper.randomIntBetween;
            let hsla = [
                [rib(s.h[0], s.h[1]),
                 rib(s.s[0], s.s[1]), 
                 rib(s.l[0], s.l[1]),
                 rib(s.a[0] * 10, s.a[1] * 10) / 10,
                ], 
                [rib(s.h[0], s.h[1]), 
                 rib(s.s[0], s.s[1]), 
                 rib(s.l[0], s.l[1]),
            ...
            this.gradientContextHSLA(ctx, hsla, gradientDirection);
        }
        if (themeSettings.gradient == "random") {
            this.gradientContext(ctx, gradient, gradientDirection);
        }
        if (themeSettings.pattern == "random" && this._theme.pattern) {
            // let patterns = await this._themeLoader.loadPatterns();
            let randomPattern = radugen.helper.getRndFromArr(this._theme.pattern);
            this.patternizeContext(ctx, await this.loadImage(randomPattern), themeSettings.patternOpacity);
        }
    }
    ...
    gradientContextHSLA(ctx, hsla, direction) {
        ctx.save();
        ctx.globalCompositeOperation = 'source-atop';
        const rnd = radugen.helper.getRndFromNum
        ...
        let [h, s, l, a] = hsla[0];
        gradient.addColorStop(0, `hsla(${h}, ${s}%, ${l}%, ${a})`);

        [h, s, l, a] = hsla[1];
        gradient.addColorStop(1, `hsla(${h}, ${s}%, ${l}%, ${a})`);
    ...
    async renderWallTiles(ctx, wallTextureSrc) {
        ...
        this._grid.iterate((tile, x, y, adjecent) => {
            if(tile.type != 0 && adjecent.top.type == 0) { tile.wall.top = true; }
            if(tile.type != 0 && adjecent.right.type == 0) { tile.wall.right = true; }
            if(tile.type != 0 && adjecent.bottom.type == 0) { tile.wall.bottom = true; }
            if(tile.type != 0 && adjecent.left.type == 0) { tile.wall.left = true; }
        ...
        if (pillarTextureImages.length) {
            ...
            this._grid.iterateNodes((x, y, node) => {
                ...
                const isStraightConnection = 
                    (node.connections.top && node.connections.bottom && !node.connections.right && !node.connections.left) ||
                    (node.connections.left && node.connections.right && !node.connections.top && !node.connections.bottom);

                const isCornerConnection = !isStraightConnection && connectionCount >= 2;

                if(isStraightConnection){
                    this.drawWalnoot(ctx, [x * this._tileResolution, y * this._tileResolution], pillarTextureImage);
                }
                else if(isCornerConnection){
                    this.drawCorner(ctx, [x * this._tileResolution, y * this._tileResolution], pillarTextureImage);
                }
                else{
    ...
    drawWall(ctx, startPos, endPos, wallTextureImg) {
        if (wallTextureImg) {
            ...
            if (xstart == xend) {
                // vertical wall
                ctx.drawImage(wallTextureImg, xstart + (width - wallThickness) / 2, ystart, wallThickness, height);
            } else {
                // horizontal wall
                ctx.drawImage(wallTextureImg, xstart, ystart + (height - wallThickness) / 2, width, wallThickness);
            }
            ctx.restore();
        }
    }
